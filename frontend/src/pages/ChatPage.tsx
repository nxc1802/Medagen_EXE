...
import type { TriageResult, ConversationMessage } from '../types'
...
// ChatMessage is aliased from the canonical ConversationMessage type
...
export default function ChatPage() {
  ...
  // Request geolocation once on mount for follow-up context
  useEffect(() => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const result = await sendFollowUp(sessionId, text, uploadedUrl, userLocation)
      setMessages(prev => prev.map(m =>
        m.id === placeholder.id
